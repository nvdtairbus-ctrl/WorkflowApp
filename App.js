...
import { Provider as PaperProvider, Card, Button, TextInput as PaperInput } from 'react-native-paper';
...
const ActiveScreen = ({ refreshTrigger, onRefresh }) => {
  ...
  const getPermitColor = (permit) => {
    // اگر پرونده پین شده باشد، رنگ قرمز
    if (permit.pinned) return '#FFEBEE';  // قرمز خیلی روشن
  ...
  const getBorderColor = (permit) => {
    // اگر پرونده پین شده باشد، حاشیه قرمز
    if (permit.pinned) return '#F44336';  // قرمز
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F7FA' }}>
      <ScrollView style={{ flex: 1, padding: 12 }}>
        {permitList.map((permit) => {
          ...
          const cardColor = getPermitColor(permit);
          const borderColor = getBorderColor(permit);
          
          return (
            <View key={permit.id} style={{ marginBottom: 12 }}>
              <TouchableOpacity
                ...
                <Card style={{ marginTop: 4, borderRadius: 16, backgroundColor: 'white', elevation: 2 }}>
                  <Card.Content>
                    ...
                    {permit.steps.map((step, idx) => (
                      <TouchableOpacity
                        key={step.id}
                        onPress={() => !step.completed && toggleStepCompletion(permit.id, step.id)}
                      ...
                      >
                        <View
                          style={{
                            ...
                            alignItems: 'center',
                            paddingVertical: 10,
                            borderBottomWidth: idx === permit.steps.length - 1 ? 0 : 1,
                    ...
                    <View style={{ flexDirection: 'row', marginTop: 16, gap: 10, flexWrap: 'wrap' }}>
                      ...
                      <Button
                        mode="outlined"
                        onPress={() => completePermit(permit.id)}
                    ...
                    <View style={{ flexDirection: 'row', marginTop: 10, gap: 10, flexWrap: 'wrap' }}>
                      ...
                      <Button
                        mode="text"
                        onPress={() => togglePin(permit.id)}
                        style={{ flex: 1 }}
                        textColor={permit.pinned ? '#F44336' : '#7F8C8D'}
                      >
                        {permit.pinned ? '📌 پین شده' : '📍 پین کن'}
                      </Button>
                      <Button
                        mode="text"
